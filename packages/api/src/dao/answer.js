...
async function createAnswer(answer = null) {
  ...
  const sSQLC =
    'INSERT INTO hq.answers (`question_id`, `option`, `date`, `time`) VALUES(?, ?, ?, ?)';
  const values = [answer.question_id, answer.option, answer.date, answer.time];
  let infoInsert;
  try {
...
async function getAnswers() {
  ...
  let rows;
  const sSQL =
    'SELECT id, ' + 'question_id, ' + '`option`, ' + '`date`, ' + '`time` ' + 'FROM hq.answers';
  try {
